"use client";
// import { getProjectDetail, projectDetail, updateProjectDetail } from "@/features/user/userSlice";
...
const page = () => {
  ...
  const filterData = project.find((e) => e._id === id);
  ...
  useEffect(() => {
    if (filterData) {
      reset({
        ...
        tasks: filterData.tasks?.map((t) => ({ name: t })) || [{ name: "" }],
        members: filterData.members?.map((m) => ({ name: m })) || [
          { name: "" },
        ],
  ...
  const onSubmit = (data) => {
    ...
    setSuccessMessage("Tasks updated SuccesFully");
    dispatch(updateProjectDetail({ data: transformedData, id }));
    window.scrollTo(0, 0);
    setTimeout(() => {
      router.push(`/${id}/detailTask`);
    }, 3000);
  };
  ...
  return (
    <>
      ...
      >
        <div
          className={`flex flex-col  p-3   w-full md:px-5 shadow-[0_3px_10px_rgb(0,0,0,0.5)] ${
            successMessage ? "opacity-0" : ""
          } `}
        >
          <p className="md:text-3xl text-xl text-center bg-gray-800 text-white py-2 ">
            Update Task
          </p>
          ...
          <input
            ...
            }    md:p-3 py-2 px-3  text-gray-900 `}
            type="text"
            placeholder="Project"
            name="project"
            ...
            <div key={`${item._id}-${index}`} className="mb-4">
              <div className="flex gap-2">
                <input
                  ...
                  className={`w-full   md:p-3 py-2 px-3  rounded-md text-gray-900 border 
              ${
          ...
          <button
            ...
            className="bg-gray-800 hover:bg-gray-700 text-white  cursor-pointer md:font-semibold py-2 px-4 rounded-md mr-4"
          >
            + Add Task
          </button>
          ...
          <input
            ...
            }   md:p-3 py-2 px-3  text-gray-800 `}
            type="text"
          ...
          <label htmlFor="members" className="py-2 text-gray-700 font-bold">
            Members Name
          </label>
          ...
          <button
            ...
            className="bg-gray-800 hover:bg-gray-700  cursor-pointer text-white md:font-semibold py-2 px-4 rounded-md mr-4"
          >
            + Add Members
          </button>
          <br />
          <input
            className="border-1 rounded-md border-gray-500 cursor-pointer   md:p-3 py-2 px-3  bg-gray-800 text-white "
            type="submit"
            value="Update "
          />
        </div>
